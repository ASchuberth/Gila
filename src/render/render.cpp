...
void GilaRender::createFramebuffers() {
  ...
  for (decltype(mImageViews.size()) i = 0; i < mImageViews.size(); i++) {
    ...
    vk::Result result = mDevice.createFramebuffer(&FramebufferInfo, nullptr, &mFramebuffers[i]);
  }
}
...
void GilaRender::createDescriptorSets() {
  ...
  for (size_t i = 0; i < mImages.size(); i++) {
    ...
    mDevice.updateDescriptorSets(1,
...
void GilaRender::createCommandBuffers() {
  ...
  for (size_t i = 0; i < mCommandBuffers.size(); i++) {
    ...
    for (const auto& buffer : mBuffers) {
      ...
      mCommandBuffers[i].bindVertexBuffers(0, 1, &buffer.second.mBuffer, offsets);
